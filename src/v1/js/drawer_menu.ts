// class
class DrawerMenu implements Observer {
    private pre_navname : string;
    constructor() {
      this.pre_navname = "";
    }
    ...
    // 메뉴 펼침
    openDrawer(){
      const layDrawerMenu = document.querySelector<HTMLDivElement>("#drawer_menu")!;
    ...
    closeDrawer() {
      const layDrawerMenu = document.querySelector<HTMLDivElement>("#drawer_menu")!;
      const closeDW = document.querySelector<HTMLButtonElement>("#btn-close-dw")!;
      if (closeDW) {
    ...
    doDrawerListener() 
    {
      document.querySelectorAll<HTMLDivElement>(".drawer-item").forEach((el) =>
      {
