...
export class RecyclerView {
  ...
  handleScroll() {
    ...
    let endIndex = Math.min(startIndex + this.options.itemCount, itemCount);
    if (scrollPosition + containerHeight >= this.container.scrollHeight - this.options.bottomBuffer) {
      const nextBatchItemCount = this.options.itemCount;
      endIndex = Math.min(endIndex + nextBatchItemCount, itemCount);
    }
    // endIndex가 itemCount를 초과하지 않는지 확인하세요.
