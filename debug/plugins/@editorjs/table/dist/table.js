...
class u {
   ...
   */
  createToolbox() {
    const t = c("div", [u.CSS.toolbox, this.cssModifier ? `${u.CSS.toolbox}--${this.cssModifier}` : ""]);
...
class A {
  ...
  constructor(t, e, o, i) {
    this.readOnly = t, this.api = e, this.data = o, this.config = i, this.wrapper = null, this.table = null, this.toolboxColumn = this.createColumnToolbox(), this.toolboxRow = this.createRowToolbox(), this.createTableWrapper(), this.hoveredRow = 0, this.hoveredColumn = 0, this.selectedRow = 0, this.selectedColumn = 0, this.tunes = {
      ...
      const n = s.target.closest(`.${r.table}`) !== null,
        h = s.target.closest(`.${r.wrapper}`) === null;
      (n || h) && this.hideToolboxes();
      const w = s.target.closest(`.${r.addRow}`),
        p = s.target.closest(`.${r.addColumn}`);
  ...
  binSearch(t, e, o, i) {
    ...
    for (; s < n - 1 && h < 10;) {
      ...
      const w = e(l),
        p = g(this.table, w);
      if (o(p)) n = l;else if (i(p)) s = l;else break;
      h++;
    }
    return l;
  }
  ...
  getData() {
    ...
    for (let e = 1; e <= this.numberOfRows; e++) {
      const o = this.table.querySelector(`.${r.row}:nth-child(${e})`),
        i = Array.from(o.querySelectorAll(`.${r.cell}`));
...
class I {
  ...
  onPaste(t) {
    const e = t.detail.data,
      ...
      s = Array.from(e.querySelectorAll("tr")).map(n => Array.from(n.querySelectorAll("th, td")).map(l => l.innerHTML));
    this.data = {
      withHeadings: o !== null,
      content: s
