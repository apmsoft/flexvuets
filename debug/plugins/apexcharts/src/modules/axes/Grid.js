...
class Grid {
  ...
  createGridMask() {
    ...
    const hasBar = type === 'bar' || type === 'rangeBar' || type === 'candlestick' || type === 'boxPlot' || w.globals.comboBarCount > 0;
    let barWidthLeft = 0;
    let barWidthRight = 0;
  ...
  _drawGridBandRect({
    ...
    let rect = graphics.drawRect(x1 - (type === 'row' ? offX : 0), y1, x2 + (type === 'row' ? offX * 2 : 0), y2, 0, color, w.config.grid[type].opacity);
    this.elg.add(rect);
  ...
  renderGrid() {
    ...
    if (!w.globals.isBarHorizontal || this.isRangeBar) {
      ...
      if (this.isRangeBar) {
        ...
        if (w.globals.yAxisScale?.[gridAxisIndex]?.result?.length > 0 && w.config.xaxis.type !== 'datetime') {
  ...
  drawGridBands(xCount, tickAmount) {
    ...
    if (w.config.grid.column.colors !== undefined && w.config.grid.column.colors.length > 0) {
      const xc = !w.globals.isBarHorizontal && w.config.xaxis.tickPlacement === 'on' && (w.config.xaxis.type === 'category' || w.config.xaxis.convertedCatToNumeric) ? xCount - 1 : xCount;
      let x1 = w.globals.padHorizontal;
      let y1 = 0;
