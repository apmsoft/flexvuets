...
export default class YAxis {
    ...
    if (w.config.yaxis[realIndex].labels.show) {
      for (let i = tickAmount; i >= 0; i--) {
        ...
        if (w.config.yaxis[realIndex].labels.rotate !== 0) {
          ...
          label.node.setAttribute('transform', `rotate(${w.config.yaxis[realIndex].labels.rotate} ${firstabelRotatingCenter.x} ${labelRotatingCenter.y})`);
        }
        l = l + labelsDivider;
      }
    }
  ...
  inversedYAxisBorder(parent) {
    ...
    if (axisBorder.show) {
      ...
      let elHorzLine = graphics.drawLine(w.globals.padHorizontal + lineCorrection + axisBorder.offsetX, this.xAxisoffX, w.globals.gridWidth, this.xAxisoffX, axisBorder.color, 0, axisBorder.height);
  ...
  setYAxisTextAlignments() {
    ...
    yaxis.forEach((y, index) => {
      ...
      if (yaxe && !yaxe.floating && yaxe.labels.align !== undefined) {
        ...
        let yAxisTexts = w.globals.dom.baseEl.querySelectorAll(`.apexcharts-yaxis[rel='${index}'] .apexcharts-yaxis-label`);
