...
export default class Helpers {
  ...
  getPathFillColor(series, i, j, realIndex) {
    ...
    let pathFill = fill.fillPath({
      ...
      fillType: w.config.series[i].data[j]?.fill?.type ? w.config.series[i].data[j]?.fill.type : Array.isArray(w.config.fill.type) ? w.config.fill.type[i] : w.config.fill.type
    });
    return pathFill;
  }
  ...
  getColumnPaths({
    ...
    pathTo = pathTo + graphics.line(x1, y2) + graphics.line(x2 - strokeWidth, y2) + graphics.line(x2 - strokeWidth, y1) + (w.config.plotOptions.bar.borderRadiusApplication === 'around' ? ' Z' : ' z');
  ...
  drawBarShadow({
    ...
    const shadowPath = graphics.move(prevX1, prevY2) + graphics.line(prevX2, prevY2) + graphics.line(currX2, currY1) + graphics.line(currX1, currY1) + graphics.line(prevX1, prevY2) + (w.config.plotOptions.bar.borderRadiusApplication === 'around' ? ' Z' : ' z');
