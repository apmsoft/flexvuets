...
class Pie {
  ...
  pieClicked(i) {
    ...
    let elPath = w.globals.dom.Paper.select(`.apexcharts-${me.chartType.toLowerCase()}-slice-${i}`).members[0];
  ...
  renderInnerDataLabels(dataLabelsConfig, opts) {
    ...
    let g = graphics.group({
      ...
      transform: `translate(${opts.translateX ? opts.translateX : 0}, ${opts.translateY ? opts.translateY : 0}) scale(${w.config.plotOptions.pie.customScale})`
    });
  ...
  revertDataLabelsInner(elem, dataLabelsConfig, event) {
    ...
    let dataLabelsGroup = w.globals.dom.baseEl.querySelector('.apexcharts-datalabels-group');
    let sliceOut = false;
    const slices = w.globals.dom.baseEl.getElementsByClassName(`apexcharts-pie-area`);
    const selectSlice = ({
      makeSliceOut,
      printLabel
    }) => {
      ...
      if (!sliceOut) {
        if (w.globals.selectedDataPoints.length && w.globals.series.length > 1) {
          if (w.globals.selectedDataPoints[0].length > 0) {
            ...
            const el = w.globals.dom.baseEl.querySelector(`.apexcharts-${this.chartType.toLowerCase()}-slice-${index}`);
