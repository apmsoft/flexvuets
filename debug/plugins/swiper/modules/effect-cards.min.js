  ...
  effectInit({
    ...
    setTranslate: () => {
      ...
      for (let l = 0; l < e.length; l += 1) {
        ...
        const g = c < 0 ? "" + (1 + (1 - u) * c) : "" + (1 - (1 - u) * c),
          x = `\n        translate3d(${p}, ${h}, ${M}px)\n        rotateZ(${r.rotate ? s ? -w : w : 0}deg)\n        scale(${g})\n      `;
        if (r.slideShadows) {
          let e = d.querySelector(".swiper-slide-shadow");
          e || (e = createShadow("cards", d)), e && (e.style.opacity = Math.min(Math.max((Math.abs(c) - .5) / .5, 0), 1));
        }
        d.style.zIndex = -Math.abs(Math.round(f)) + e.length;
        effectTarget(r, d).style.transform = x;
      }
    },
    setTransition: e => {
      const a = t.slides.map(e => getSlideTransformEl(e));
      a.forEach(t => {
