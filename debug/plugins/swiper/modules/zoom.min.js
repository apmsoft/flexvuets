...
function Zoom(e) {
  ...
  function E() {
    const e = t.params.zoom,
      i = d.imageWrapEl.getAttribute("data-swiper-zoom") || e.maxRatio;
  ...
  function f(e) {
    if ("mouse" === e.pointerType && c.splice(0, c.length), !v(e)) return;
    ...
    if (o = !1, n = !1, c.push(e), !(c.length < 2)) {
      if (o = !0, d.scaleStart = h(), !d.slideEl) {
        ...
        if (a && (a = a.querySelectorAll("picture, img, svg, canvas, .swiper-zoom-target")[0]), d.imageEl = a, d.imageWrapEl = a ? elementParents(d.imageEl, `.${i.containerClass}`)[0] : void 0, !d.imageWrapEl) return void (d.imageEl = void 0);
        d.maxRatio = E();
      }
      if (d.imageEl) {
        const [e, t] = function () {
          ...
          return [(c[0].pageX + (c[1].pageX - c[0].pageX) / 2 - e.x - s.scrollX) / l, (c[0].pageY + (c[1].pageY - c[0].pageY) / 2 - e.y - s.scrollY) / l];
        }();
  ...
  function Y(e) {
    ...
    u.isMoved || (u.width = d.imageEl.offsetWidth, u.height = d.imageEl.offsetHeight, u.startX = getTranslate(d.imageWrapEl, "x") || 0, u.startY = getTranslate(d.imageWrapEl, "y") || 0, d.slideWidth = d.slideEl.offsetWidth, d.slideHeight = d.slideEl.offsetHeight, d.imageWrapEl.style.transitionDuration = "0ms");
  ...
  function z(e) {
    const i = t.zoom,
      a = t.params.zoom;
    if (!d.slideEl) {
      e && e.target && (d.slideEl = e.target.closest(`.${t.params.slideClass}, swiper-slide`)), d.slideEl || (t.params.virtual && t.params.virtual.enabled && t.virtual ? d.slideEl = elementChildren(t.slidesEl, `.${t.params.slideActiveClass}`)[0] : d.slideEl = t.slides[t.activeIndex]);
      let i = d.slideEl.querySelector(`.${a.containerClass}`);
      i && (i = i.querySelectorAll("picture, img, svg, canvas, .swiper-zoom-target")[0]), d.imageEl = i, d.imageWrapEl = i ? elementParents(d.imageEl, `.${a.containerClass}`)[0] : void 0;
    }
    if (!d.imageEl || !d.imageWrapEl) return;
    let r, o, n, m, c, p, g, h, v, f, x, X, Y, y, z, C, M, w;
    ...
    const b = E();
    i.scale = W || b, l = W || b, !e || 1 === l && W ? (g = 0, h = 0) : (M = d.slideEl.offsetWidth, w = d.slideEl.offsetHeight, n = elementOffset(d.slideEl).left + s.scrollX, m = elementOffset(d.slideEl).top + s.scrollY, c = n + M / 2 - r, p = m + w / 2 - o, v = d.imageEl.offsetWidth, f = d.imageEl.offsetHeight, x = v * i.scale, X = f * i.scale, Y = Math.min(M / 2 - x / 2, 0), y = Math.min(w / 2 - X / 2, 0), z = -Y, C = -y, g = c * i.scale, h = p * i.scale, g < Y && (g = Y), g > z && (g = z), h < y && (h = y), h > C && (h = C)), W && 1 === i.scale && (d.originX = 0, d.originY = 0), d.imageWrapEl.style.transitionDuration = "300ms", d.imageWrapEl.style.transform = `translate3d(${g}px, ${h}px,0)`, d.imageEl.style.transitionDuration = "300ms", d.imageEl.style.transform = `translate3d(0,0,0) scale(${i.scale})`;
  }
  function C() {
    const e = t.zoom,
      i = t.params.zoom;
    if (!d.slideEl) {
      t.params.virtual && t.params.virtual.enabled && t.virtual ? d.slideEl = elementChildren(t.slidesEl, `.${t.params.slideActiveClass}`)[0] : d.slideEl = t.slides[t.activeIndex];
      let e = d.slideEl.querySelector(`.${i.containerClass}`);
      e && (e = e.querySelectorAll("picture, img, svg, canvas, .swiper-zoom-target")[0]), d.imageEl = e, d.imageWrapEl = e ? elementParents(d.imageEl, `.${i.containerClass}`)[0] : void 0;
    }
  ...
  function W() {
    ...
    t.wrapperEl.addEventListener("pointerdown", f, i), t.wrapperEl.addEventListener("pointermove", x, a), ["pointerup", "pointercancel", "pointerout"].forEach(e => {
  ...
  function b() {
    ...
    t.wrapperEl.removeEventListener("pointerdown", f, i), t.wrapperEl.removeEventListener("pointermove", x, a), ["pointerup", "pointercancel", "pointerout"].forEach(e => {
