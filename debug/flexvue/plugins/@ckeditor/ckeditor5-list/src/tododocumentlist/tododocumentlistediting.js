  ...
  init() {
    ...
    documentListEditing.registerDowncastStrategy({
      ...
      createElement(writer, modelElement, {
        ...
        const viewElement = writer.createEmptyElement('input', Object.assign(Object.assign({
          type: 'checkbox'
    ...
    editing.mapper.registerViewToModelLength('input', viewElement => {
      if (viewElement.getAttribute('type') == 'checkbox' && viewElement.findAncestor({
        classes: 'todo-list__label'
      })) {
        return 0;
      }
    ...
    documentListEditing.on('checkElement', (evt, {
      modelElement,
      viewElement
    }) => {
    ...
    documentListEditing.on('postFixer', (evt, {
      listNodes,
      writer
    }) => {
      for (const {
