...
*/
import { getOptimalPosition, global, toUnit, Rect } from "../../../ckeditor5-editor-classic/src/index.js/ckeditor5-utils";
...
export default class AutocompleteView extends SearchTextView {
  ...
  constructor(locale, config) {
    ...
    this.resultsView.extendTemplate({
      attributes: {
        class: [bindResultsView.if('isVisible', 'ck-hidden', value => !value), bindResultsView.to('_position', value => `ck-search__results_${value}`)],
        style: {
    ...
    this.filteredView.on('execute', (evt, {
      value
    }) => {
 ...
 */
AutocompleteView.defaultResultsPositions = [fieldRect => {
  return {
