...
import { toMap } from "../../../ckeditor5-editor-classic/src/index.js/ckeditor5-utils";
...
export function stringify(node, selectionOrPositionOrRange = null, markers = null) {
  ...
  downcastDispatcher.on('attribute', (evt, data, conversionApi) => {
    if (data.item instanceof ModelSelection || data.item instanceof DocumentSelection || data.item.is('$textProxy')) {
      const converter = wrap((modelAttributeValue, {
        writer
      }) => {
  ...
  downcastDispatcher.on('addMarker', insertUIElement((data, {
    writer
  }) => {
  ...
  let data = viewStringify(viewRoot, viewDocument.selection, {
...
export function parse(data, schema, options = {}) {
  ...
  if (viewSelection) {
    ...
    selection = new ModelSelection(ranges, {
