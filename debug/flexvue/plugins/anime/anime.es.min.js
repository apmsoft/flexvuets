...
function spring(e, t) {
  function n(e) {
    var n = t ? t * e / 1e3 : e;
    return n = l < 1 ? Math.exp(-n * l * c) * (f * Math.cos(g * n) + m * Math.sin(g * n)) : (f + m * n) * Math.exp(-n * c), 0 === e || 1 === e ? e : 1 - n;
  }
  function r() {
    var t = cache.springs[e];
    if (t) return t;
    for (var r = 1 / 6, a = 0, i = 0;;) if (a += r, 1 === n(a)) {
      if (i++, i >= 16) break;
    } else i = 0;
    var o = a * r * 1e3;
  ...
  var a = parseEasingParameters(e),
    ...
    o = minMax(is.und(a[1]) ? 100 : a[1], .1, 100),
    s = minMax(is.und(a[2]) ? 10 : a[2], .1, 100),
    u = minMax(is.und(a[3]) ? 0 : a[3], .1, 100),
    c = Math.sqrt(o / i),
    l = s / (2 * Math.sqrt(o * i)),
    g = l < 1 ? c * Math.sqrt(1 - l * l) : 0,
    f = 1,
    m = l < 1 ? (l * c - u) / g : -u + c;
  return t ? n : r;
}
...
function convertPxToUnit(e, t, n) {
  var r = getUnit(t);
  if (arrayContains([n, "deg", "rad", "turn"], r)) return t;
  var a = cache.CSS[t + n];
  if (!is.und(a)) return a;
  var i = 100,
    o = document.createElement(e.tagName),
    s = e.parentNode && e.parentNode !== document ? e.parentNode : document.body;
  s.appendChild(o), o.style.position = "absolute", o.style.width = i + n;
  var u = i / o.offsetWidth;
  s.removeChild(o);
  var c = u * parseFloat(t);
  return cache.CSS[t + n] = c, c;
}
...
function parseTargets(e) {
  var t = e ? flattenArray(is.arr(e) ? e.map(toArray) : toArray(e)) : [];
  return filterArray(t, function (e, t, n) {
...
function normalizePropertyTweens(e, t) {
  ...
  return i.map(function (e, n) {
    ...
    return is.und(r.delay) && (r.delay = n ? 0 : t.delay), is.und(r.endDelay) && (r.endDelay = n === i.length - 1 ? t.endDelay : 0), r;
  }).map(function (e) {
  ...
  n.forEach(function (e) {
    for (var n in t) {
      var r = getFunctionValue(t[n], e),
        ...
        s = i || getUnit(o),
        u = getRelativeValue(validateValue(r, s), o),
        c = getAnimationType(a, n);
...
function anime(e) {
  ...
  function n() {
    var e = h.direction;
    "alternate" !== e && (h.direction = "normal" !== e ? "normal" : "reverse"), h.reversed = !h.reversed, g.forEach(function (e) {
  ...
  function a() {
    f = 0, m = r(h.currentTime) * (1 / anime.speed);
  }
  function i(e, t) {
    t && t.seek(e - t.timelineOffset);
  }
  function o(e) {
    if (h.reversePlayback) for (var t = p; t--;) i(e, g[t]);else for (var n = 0; n < p; n++) i(e, g[n]);
  }
  function s(e) {
    for (var t = 0, n = h.animations, r = n.length; t < r;) {
      var a = n[t],
        i = a.animatable,
        o = a.tweens,
        s = o.length - 1,
        u = o[s];
      s && (u = filterArray(o, function (t) {
        return e < t.end;
      })[0] || u);
  ...
  function l(e) {
    var a = h.duration,
      ...
      l = a - h.endDelay,
      p = r(e);
    h.progress = minMax(p / a * 100, 0, 100), h.reversePlayback = p < h.currentTime, g && o(p), !h.began && h.currentTime > 0 && (h.began = !0, u("begin")), !h.loopBegan && h.currentTime > 0 && (h.loopBegan = !0, u("loopBegin")), p <= i && 0 !== h.currentTime && s(0), (p >= l && h.currentTime !== a || !a) && s(a), p > i && p < l ? (h.changeBegan || (h.changeBegan = !0, h.changeCompleted = !1, u("changeBegin")), u("change"), s(p)) : h.changeBegan && (h.changeCompleted = !0, h.changeBegan = !1, u("changeComplete")), h.currentTime = minMax(p, 0, a), h.began && u("update"), e >= a && (m = 0, c(), h.remaining ? (f = d, u("loopComplete"), h.loopBegan = !1, "alternate" === h.direction && n()) : (h.paused = !0, h.completed || (h.completed = !0, u("loopComplete"), u("complete"), !h.passThrough && "Promise" in window && (v(), t(h)))));
  }
  void 0 === e && (e = {});
  var g,
    f = 0,
    ...
    p = 0,
    v = null,
    h = createNewInstance(e);
  t(h);
  return h.reset = function () {
    var e = h.direction;
    h.passThrough = !1, h.currentTime = 0, h.progress = 0, h.paused = !0, h.began = !1, h.loopBegan = !1, h.changeBegan = !1, h.completed = !1, h.changeCompleted = !1, h.reversePlayback = !1, h.reversed = "reverse" === e, h.remaining = h.loop, g = h.children, p = g.length;
    for (var t = p; t--;) h.children[t].reset();
    (h.reversed && !0 !== h.loop || "alternate" === e && 1 === h.loop) && h.remaining++, s(h.reversed ? h.duration : 0);
  }, h._onDocumentVisibility = a, h.set = function (e, t) {
    return setTargetsValue(e, t), h;
  }, h.tick = function (e) {
    d = e, f || (f = d), l((d + (m - f)) * anime.speed);
  }, h.seek = function (e) {
    l(r(e));
  }, h.pause = function () {
    h.paused = !0, a();
  }, h.play = function () {
    h.paused && (h.completed && h.reset(), h.paused = !1, activeInstances.push(h), a(), engine());
  }, h.reverse = function () {
    n(), h.completed = !h.reversed, a();
  }, h.restart = function () {
    h.reset(), h.play();
  }, h.remove = function (e) {
    var t = parseTargets(e);
    removeTargetsFromInstance(t, h);
...
function stagger(e, t) {
  ...
  return function (e, t, h) {
    ...
    var I = l ? (f - g) / v : g;
    return d + I * (Math.round(100 * p[t]) / 100) + m;
  };
}
function timeline(e) {
  void 0 === e && (e = {});
  ...
  return t.duration = 0, t.add = function (n, r) {
    ...
    var i = activeInstances.indexOf(t),
      o = t.children;
    i > -1 && activeInstances.splice(i, 1);
    ...
    var c = t.duration;
    u.autoplay = !1, u.direction = t.direction, u.timelineOffset = is.und(r) ? c : getRelativeValue(r, c), a(t), t.seek(u.timelineOffset);
    var l = anime(u);
    a(l), o.push(l);
    var g = getInstanceTimings(o, e);
...
var defaultInstanceSettings = {
  ...
  bezier = function () {
    ...
    function s(e, t, n, s) {
      function l(t) {
        for (var r = 0, s = 1, l = u - 1; s !== l && g[s] <= t; ++s) r += c;
        --s;
        var f = (t - g[s]) / (g[s + 1] - g[s]),
          m = r + f * c,
          d = a(m, e, n);
        return d >= .001 ? o(t, m, e, n) : 0 === d ? m : i(t, r, r + c, e, n);
      }
      if (0 <= e && e <= 1 && 0 <= n && n <= 1) {
        var g = new Float32Array(u);
        if (e !== t || n !== s) for (var f = 0; f < u; ++f) g[f] = r(f * c, e, n);
        return function (a) {
          return e === t && n === s ? a : 0 === a || 1 === a ? a : r(l(a), t, s);
        };
      }
    }
    var u = 11,
  ...
  engine = function () {
    function e() {
      r || isDocumentHidden() && anime.suspendWhenDocumentHidden || !(activeInstances.length > 0) || (r = requestAnimationFrame(t));
    }
    function t(e) {
      for (var n = activeInstances.length, a = 0; a < n;) {
    ...
    function n() {
      anime.suspendWhenDocumentHidden && (isDocumentHidden() ? r = cancelAnimationFrame(r) : (activeInstances.forEach(function (e) {
        return e._onDocumentVisibility();
      }), engine()));
    }
    var r;
    return "undefined" != typeof document && document.addEventListener("visibilitychange", n), e;
  }();
