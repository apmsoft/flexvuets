...
const onReady = () => {
  ...
  // AES 암호화
  const encrypt_str = CryptoES.AES.encrypt(JSON.stringify(test_json), hash_key).toString();
  Log.d('AES encrypt', encrypt_str);
  // AES 복호화
  let decrypt_str = CryptoES.AES.decrypt(encrypt_str, hash_key).toString(CryptoES.enc.Utf8);
  Log.d('AES decrypt String', decrypt_str);
  // json parse
  console.log(' JSON Parse', JSON.parse(decrypt_str));
  Log.d("============= MD5 ================");
  const md5_encrypt_str = CryptoES.MD5("1234**").toString();
  Log.d('md5_encrypt_str', md5_encrypt_str);
  Log.d("============= SHA256 ================");
  const sha256_encrypt_str = CryptoES.SHA256("4567**").toString();
  Log.d('sha256_encrypt_str', sha256_encrypt_str);
  Log.d("============= SHA256 + BASE64 [Encode | Decode ] ================");
  const sha256_wordarray = CryptoES.SHA256("4567**");
